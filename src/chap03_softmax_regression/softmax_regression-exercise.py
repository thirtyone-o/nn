...
# c="b" 设置颜色为蓝色，marker="+" 设置标记为加号
# C1[:, 0] 表示类别1所有样本的第一个特征值（x坐标）
# C1[:, 1] 表示类别1所有样本的第二个特征值（y坐标）
plt.scatter(C2[:, 0], C2[:, 1], c="g", marker="o")
# c="g" 设置颜色为绿色，marker="o" 设置标记为圆圈
# C2[:, 0] 表示类别2所有样本的第一个特征值（x坐标）
# C2[:, 1] 表示类别2所有样本的第二个特征值（y坐标）
plt.scatter(C3[:, 0], C3[:, 1], c="r", marker="*")

# c="r" 设置颜色为红色，marker="*" 设置标记为星号
# C3[:, 0] 表示类别3所有样本的第一个特征值（x坐标）
# C3[:, 1] 表示类别3所有样本的第二个特征值（y坐标）

x = np.arange(0.0, 10.0, 0.1) # 创建从0.0到10.0，步长为0.1的x坐标数组
y = np.arange(0.0, 10.0, 0.1) # 创建从0.0到10.0，步长为0.1的y坐标数组
# 生成网格点，用于绘制决策边界或等高线
